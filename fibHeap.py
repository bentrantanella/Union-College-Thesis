...
class FibonacciHeap:
    ...
    class Node:
        ...
        def connects(self):
            return str(self.parent) + ", " + str(self.child) + ", " + str(self.left) + ", " + str(self.right)
    ...
    def iterateV2(self, head):
        print("HEAD: " + str(head))
        runner = stop = head
        toReturn = [runner]
        runner = runner.right
        while runner != stop:
            toReturn.append(runner)
            runner = runner.right
            print(str(runner))
    ...
    def extract_min(self):
        #print(self.min_node.value)
        z = self.min_node
        if z is not None:
            if z.child is not None:
                # attach child nodes to root list
                children = [x for x in self.iterate(z.child)]
                #children = self.iterateV2(z.child)
    ...
    def merge(self, h2):
        ...
        h2.root_list.left.right = H.root_list
